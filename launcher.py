...
class Launcher(tk.Tk):
    ...
    def _wait_lab_targets_ready(self, timeout=60):
        ...
        reporten 'healthy' antes de disparar el escaneo. Corrige la condicion
        de carrera documentada en Pendientes_Correccion_TFI.txt punto 1.3
        (172.20.0.10 nunca detectado por Nmap: el escaneo arrancaba antes de
        que el contenedor terminara de levantar). Si lab-targets no esta
        corriendo (compose file inexistente o sin containers), no bloquea:
        loguea un aviso y deja seguir, para no romper setups sin lab-targets.
        """
